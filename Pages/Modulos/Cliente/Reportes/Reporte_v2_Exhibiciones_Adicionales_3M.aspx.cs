...
{
    public partial class Reporte_v2_Exhibiciones_Adicionales_3M : System.Web.UI.Page
    {
        ...
        private void UpdateProgressContext2()
        {
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                progress.CurrentOperationText = "Reporte Sugeridos de Productos Canal AASS";
        ...
        protected void llenarCategoria()
        {
            ...
            icompany = Convert.ToInt32(this.Session["companyid"].ToString());
            canal = this.Session["Canal"].ToString();
            dt = oCoon.ejecutarDataTable("UP_WEBXPLORA_CLIE_V2_OBTENERCADENASFAMILIASCATEGORIA_3M", icompany, canal, Report, '0');
      ...
      protected void llenarPuntoVenta()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                cmb_tienda.DataSource = dt;
                cmb_tienda.DataValueField = "id_PointOfsale";
                cmb_tienda.DataTextField = "pdv_Name";
                cmb_tienda.DataBind();
                cmb_tienda.Items.Insert(0, new RadComboBoxItem("--Todas--", "0"));
            }

        }

      protected void llenarMPOP()
      {
          DataTable dt = null;
          icompany = Convert.ToInt32(this.Session["companyid"].ToString());
          canal = this.Session["Canal"].ToString();
          dt = oCoon.ejecutarDataTable("WEB_XPLORA_CLIEV2_OBTENERMATERIALPOP_CLIENTE_3M", icompany);

          if (dt.Rows.Count > 0)
          {
              cmbMPOP.DataSource = dt;
              cmbMPOP.DataValueField = "id_MPointOfPurchase";
              cmbMPOP.DataTextField = "POP_name";
              cmbMPOP.DataBind();
              cmbMPOP.Items.Insert(0, new RadComboBoxItem("--Todas--", "0"));
          }
        ...
        private void ObtenerInformeSug_Producto()
        {
            ...
            try
            {
                ...
                ReporExhAdi.Visible = true;
                ReporExhAdi.ZoomMode = Microsoft.Reporting.WebForms.ZoomMode.FullPage;
                ReporExhAdi.ServerReport.ReportPath = "/Reporte_Precios_V1/Reporte_Exhibiciones_Adicionales_3M";


                String strConnection = ConfigurationManager.AppSettings["SERVIDOR_REPORTING_SERVICES"];

                ReporExhAdi.ServerReport.ReportServerUrl = new Uri(strConnection);
                ReporExhAdi.ServerReport.ReportServerCredentials = new CFG.Tools.ReportServerNetCredentials();
                ...
                parametros.Add(new Microsoft.Reporting.WebForms.ReportParameter("MaterialPOP", cmb_categoria.SelectedValue));
                parametros.Add(new Microsoft.Reporting.WebForms.ReportParameter("PuntoVenta", cmb_categoria.SelectedValue));
                ...
                ReporExhAdi.ServerReport.SetParameters(parametros);
            }
            catch (Exception ex)
            {
