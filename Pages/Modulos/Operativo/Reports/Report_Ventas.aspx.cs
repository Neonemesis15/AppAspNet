...
{
    public partial class Report_Ventas : System.Web.UI.Page
    {
        ...
        protected void gv_ventas_UpdateCommand(object source, GridCommandEventArgs e)
        {
            try
            {
                ...
                string pedido = ArrayEditorValue[0].ToString();
                string ingreso = ArrayEditorValue[1].ToString();
                string venta = ArrayEditorValue[2].ToString();
                DateTime Fec_reg_bd = Convert.ToDateTime((ArrayEditorValue[3] as RadDateTimePicker).SelectedDate);

                Ocoon.ejecutarDataReader("UP_WEBXPLORA_OPE_ACTUALIZAR_REPORTE_VENTAS_SANFERNDO", iid_det, pedido, ingreso, venta, Fec_reg_bd, Session["sUser"].ToString(), DateTime.Now, ckvalidado.Checked);
